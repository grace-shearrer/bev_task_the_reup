# prediction error taste task. 4/27/2022
...
scan_trigger_text = visual.TextStim(win, text='During this scan you are going to get tastes of milkshake. Please pay attention to the logos and the tastes', pos=(0, 0))
...
def run_block():
    ...
    for trial in range(ntrials):
        ...
        visual_stim.setImage(stim_images[0])#set which image appears
        print(trial)
        print('condition %d'%trialcond[trial])
        print('showing image: %s'%stim_images[0])
        ...
        t = clock.getTime()
        logging.log("image=%s"%stim_images[0], logging.DATA)
        ratings_and_onsets.append(["image=%s"%stim_images[0],t])
        logging.flush()

        while clock.getTime()<(trialdata['onset']+cue_time):#show blank
            pass
